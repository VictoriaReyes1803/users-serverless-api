...
import { noContent, badRequest, internalError, errorResponse } from '../utils/response';
...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  try {
    ...
    await service.deleteUser(parsed.data.id);
    return noContent();
  } catch (err) {
    if (isAppError(err)) {
      return errorResponse(err.statusCode, err.message, err.details);
    }
    process.stderr.write(`[deleteUser] Unexpected error: ${String(err)}\n`);
